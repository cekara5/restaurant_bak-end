...
export function getOpeningDetails(
    ...
    const closingHrs = todaysWorkingTime.closingTime.split(':')[0].charAt(0) === '0' ? // ako je closingTime "02:00" dodaj +24sata jer je to sledeci dan
        parseInt(todaysWorkingTime.closingTime.split(':')[0]) + 24 :
        parseInt(todaysWorkingTime.closingTime.split(':')[0]);
    const closingMin = parseInt(todaysWorkingTime.closingTime.split(':')[1]);

    const openingInMins = openingHrs * 60 + openingMin;
    const closingInMins = closingHrs * 60 + closingMin;
    ...
    if (isOpened) {
        ...
        if (!nonWordkingDayToday) { // ako nije neradni dan
            if (!nonWordkingDayYesterday && currentTimeInMins < openingInMins) { // proveriti da li je u toku radno vreme od prethodnog dana: npr. sada je cetvrtak 01:00, a restoran sredom radi 10:00-02:00 => treba da vrati true
